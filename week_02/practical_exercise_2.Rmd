...
pacman::p_load(lme4, tidyverse, boot)
library(lme4)
```
...
#additional information
ls.str(politeness) #general information
summary(politeness$total_duration)
```
...
```{r}
# We just filter out anyone who isn't subject F1, everything else is the same
new_polite <- politeness %>% filter(
  subject=="F1"
)

# Here, we create a new dataframe by adding a new column to the old dataframe. The new column is identical to the "scenario" column, except that it is coded as a factor instead of an integer.
new_polite <- new_polite %>% 
  mutate(scenario_factor = as.factor(scenario))
class(new_polite$scenario_factor)

factor_model <- lm(f0mn ~ scenario_factor, data = new_polite)
integer_model <- lm(f0mn ~ scenario, data = new_polite)
summary(factor_model) 
summary(integer_model)

# model matrices 
model.matrix(factor_model)
model.matrix(integer_model)
...
```{r}
#plot 
subplot <- ggplot(data = politeness, aes(x = scenario, y = f0mn, color = attitude)) + geom_point()+ facet_wrap(~subject);print(subplot)
```
...
```{r}
# 1) i-iv, making models
model_i <- lm(f0mn ~ gender, data = politeness);model_i
model_ii <- lmer(f0mn ~ gender + (1|scenario),data = politeness);model_ii
model_iii <- lmer(f0mn ~ gender + (1|subject),data = politeness);model_iii
model_iv <- lmer(f0mn ~ gender + (1|scenario) + (1|subject), data = politeness);model_iv
```

### Comparing models
```{r}
# 2) v, comparing models 
AIC(model_i, model_ii, model_iii, model_iv)
...
MuMIn::r.squaredGLMM(model_ii)
MuMIn::r.squaredGLMM(model_iii) 
MuMIn::r.squaredGLMM(model_iv) 
```
...
model_gender_mean <- lm(f0mn_mean ~ gender, data= df_2,)
summary(model_gender_mean)

#qq-plot of single-level-model
qqnorm(resid(model_gender_mean))
qqline(resid(model_gender_mean))

#qq-plot of model 1
qqnorm(resid(model_i))
qqline(resid(model_i))


#quantile-quantile plot for the residuals of the multilevel model with two intercepts
plot(model_iv)

qqnorm(resid(model_iv))
qqline(resid(model_iv))
```
...
Having a little trouble with this one. I will come back stronger.
...
model_v <- lmer(f0mn ~ gender + attitude + (1|subject) + (1|scenario), data=politeness)
summary(model_v)

#model 6 with an interaction between attitude and gender
model_vi <- lmer(f0mn ~ gender + attitude+ gender*attitude + (1|subject) + (1|scenario), data=politeness)
summary(model_vi)
...
model_iv <- lmer(f0mn ~ gender + (1|subject) + (1|scenario), data=politeness)
summary(model_iv)
sigma(model_iv)  #30.66
...
model_vi <- lmer(f0mn ~ gender + attitude+ gender*attitude + (1|subject) + (1|scenario), data=politeness)
summary(model_vi)
sigma(model_vi)  #29.76

#comparing AIC values
AIC(model_iv, model_v, model_vi)

#comparing conditional R2 within the second levels
MuMIn::r.squaredGLMM(model_iv) #R2c = 0.8077964
MuMIn::r.squaredGLMM(model_v)  #R2c = 0.8196777
MuMIn::r.squaredGLMM(model_vi) #R2c = 0.8192531
```
...
##### The model
Based on the previous analysis of the data and modeling, the best model would be model_vi, f0mn ~ gender + attitude+ gender*attitude + (1|subject) + (1|scenario).
...
The summary output of model_vi shows how going from female (with an intercept of 254.193 hz) to male, the pitch of voice decreases by 117.983 hz. When going from the attitude "informal" to "polite" the pitch of voice decreases by 15.646 hz.
      - Females in informal attitude condition has a pitch of 255.618 (intercept)
      - Females in polite attitude condition has a pitch of 238.426 i.e. (255.618-17.192). 
      - Males in informal attitude condition has a pitch of 136.21 hz i.e. (255.618-118.232)
      - Males in polite attitude condition has a pitch of 120.564 hz i.e. (255.618-(-118.232-17.192))
  

      
According to the interaction effect, males pitch decreases with LESS than for Females i.e 5,544 hz less in the polite condition. Thus, Korean men's pitch when they are polite are higher than Korean women's pitch when they are polite, in relation to them selves (as we have modelled indvidual intercepts for each subject). So the change in pitch is lower for Males than for Females in the polite condition.
...
##### Variance components
This model has a sigma = 29.75684, an AIC = 2072.618, and a R2c = 0.8192531. The model has a slightly better AIC value than the other models, but a slightly worse sigma and R2c than model_v. 

##### Q-Q-plot of model
When comparing a Q-Q-plot from model 6 and Q-Q-plot from model_v, the values seem to fit the line slightly better for model_vi, but it is still has a right skewed tail. 

Conceptually model_vi would make sense, as one could argue that pitch would get affected depending on both gender and attitude (the interaction). 
  
```{r}
#model_vi with an interaction between attitude and gender
model_vi <- lmer(f0mn ~ gender + attitude+ gender*attitude + (1|subject) + (1|scenario), data=politeness)
summary(model_vi)

qqnorm(residuals(model_vi))
qqline(residuals(model_vi))

#for comparison 
qqnorm(resid(model_v))
qqline(resid(model_v))
